...
class Bot(BaseConnection):
    ...
    def get_chats(
        ...
        1. Получайте события ``bot_added``, ``bot_started``,
           ``bot_removed`` и ``bot_stopped`` — через Webhook
           (``bot.subscribe_webhook(url, update_types=[
           UpdateType.BOT_ADDED, UpdateType.BOT_STARTED,
           UpdateType.BOT_REMOVED, UpdateType.BOT_STOPPED])``) или
           опрашивая ``bot.get_updates()`` в собственном цикле.
    ...
    def set_commands(self, *commands: BotCommand) -> SettedCommands:
        """
        ...
        return SetCommands(bot=self, commands=list(commands)).fetch()
