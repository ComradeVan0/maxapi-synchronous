...
def _make_mock_session():
    """Создаёт замоканную requests-сессию для upload-запросов."""
...
class TestUploadFileSizeCheck:
    ...
    def test_warns_for_too_big_file(self, caplog, tmp_path):
        ...
        with (
            ...
            result = self._upload(file_path)

        assert result == '{"token":"t"}'
        assert len(caplog.records) == 1
        assert "big.png" in caplog.records[0].getMessage()

    def test_no_warning_for_small_file(self, caplog, tmp_path):
    ...
    def test_real_file_size_is_read_from_fs(self, caplog, tmp_path):
...
class TestUploadFileBufferSizeCheck:
    ...
    def _upload(self, buffer, filename):
    ...
    def test_warns_for_too_big_buffer(self, caplog):
    ...
    def test_no_warning_for_small_buffer(self, caplog):
        ...
        with (
            ...
            result = self._upload(b"\x00" * TINY_LIMIT, "small.png")
