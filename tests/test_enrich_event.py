...
class TestResolveChat:
    ...
    def test_message_created_falls_back_to_recipient_chat_id(
        ...
        _resolve_chat(fixture_message_created, bot)

        expected_chat_id = fixture_message_created.message.recipient.chat_id
        bot.get_chat_by_id.assert_called_once_with(expected_chat_id)
        assert fixture_message_created.chat is fake_chat

    def test_message_created_no_chat_id_anywhere_skips_fetch(
        self, bot, fixture_message_created
    ):
    ...
    def test_message_edited_falls_back_to_recipient_chat_id(
        ...
        _resolve_chat(fixture_message_edited, bot)

        expected_chat_id = fixture_message_edited.message.recipient.chat_id
        bot.get_chat_by_id.assert_called_once_with(expected_chat_id)

    def test_message_callback_falls_back_to_recipient_chat_id(
        ...
        _resolve_chat(fixture_message_callback, bot)

        expected_chat_id = fixture_message_callback.message.recipient.chat_id
        bot.get_chat_by_id.assert_called_once_with(expected_chat_id)

    def test_message_callback_none_message_skips_fetch(
        self, bot, fixture_message_callback
    ):
...
class TestResolveFromUser:
    ...
    def test_message_callback_sets_callback_user(
        self, bot, fixture_message_callback
    ):
        _resolve_from_user(fixture_message_callback, bot)
        assert (
            fixture_message_callback.from_user
            is fixture_message_callback.callback.user
        )

    def test_message_removed_chat_type_fetches_member(
        ...
        bot.get_chat_member = MagicMock(return_value=fake_member)

        _resolve_from_user(fixture_message_removed, bot)

        bot.get_chat_member.assert_called_once_with(
    ...
    def test_message_removed_dialog_type_sets_chat(
        self, bot, fixture_message_removed
    ):
        """DIALOG-тип — from_user = chat."""
        fake_chat = _make_chat(ChatType.DIALOG)
        fixture_message_removed.chat = fake_chat
        bot.get_chat_member = MagicMock()
    ...
    def test_message_removed_no_chat_skips_from_user(
        self, bot, fixture_message_removed
    ):
    ...
    def test_message_removed_get_chat_member_max_api_error_swallowed(
        self, bot, fixture_message_removed
    ):
    ...
    def test_message_removed_get_chat_member_max_connection_swallowed(
        self, bot, fixture_message_removed
    ):
    ...
    def test_user_removed_with_admin_id_fetches_member(
    ...
    def test_user_removed_without_admin_id_skips_member(
        self, bot, fixture_user_removed
    ):
    ...
    def test_user_removed_api_error_logs_and_keeps_from_user_none(
    ...
    def test_user_removed_connection_error_logs_and_keeps_from_user_none(
        self, bot, fixture_user_removed, caplog
    ):
        fixture_user_removed.admin_id = 9999
        bot.get_chat_member = MagicMock(
    ...
    def test_events_with_user_attr_set_from_user(
...
class TestEnrichEvent:
    ...
    def test_auto_requests_false_message_created_uses_lazy_chat_ref(
        ...
        fetched_chat = result.chat.fetch()
    ...
    def test_auto_requests_false_message_created_without_chat_id(
        self, bot, fixture_message_created
    ):
    ...
    def test_auto_requests_false_message_created_from_user_fetch_is_noop(
        self, bot, fixture_message_created
    ):
        ...
        result = enrich_event(fixture_message_created, bot)
        fetched_user = result.from_user.fetch()

        assert fetched_user is fixture_message_created.message.sender

    def test_auto_requests_false_message_answer_still_works(
        self, bot, fixture_message_created
    ):
        ...
        result = enrich_event(fixture_message_created, bot)
        result.message.answer(text="hello")

        bot.send_message.assert_called_once()

    def test_auto_requests_false_message_removed_fetches_lazily(
        self,
        bot,
        ...
        result = enrich_event(fixture_message_removed, bot)
        ...
        fetched_from_user = result.from_user.fetch()
    ...
    def test_auto_requests_false_user_removed_from_user_fetches_lazily(
        self, bot, fixture_user_removed
    ):
        ...
        fixture_user_removed.admin_id = 9999
        bot.auto_requests = False
        bot.get_chat_member = MagicMock(return_value=fake_admin)

        result = enrich_event(fixture_user_removed, bot)
    ...
    def test_auto_requests_false_message_removed_dialog_fetch(
        self,
        bot,
        fixture_message_removed,
    ):
    ...
    def test_auto_requests_false_message_removed_without_chat(
        self,
        bot,
        fixture_message_removed,
    ):
        ...
        bot.auto_requests = False
        bot.get_chat_by_id = MagicMock(return_value=None)
        bot.get_chat_member = MagicMock()
    ...
    def test_auto_requests_false_message_removed_unknown_chat_type(
        self, bot, fixture_message_removed
    ):
    ...
    def test_auto_requests_false_user_removed_without_admin_keeps_none(
        self, bot, fixture_user_removed
    ):
    ...
    def test_auto_requests_false_bot_removed_keeps_chat_none(
        self, bot, fixture_bot_removed
    ):
        """BotRemoved не должен получать lazy chat ref."""
        bot.auto_requests = False

        result = enrich_event(fixture_bot_removed, bot)
    ...
    def test_full_pipeline_bot_removed(self, bot, fixture_bot_removed):
    ...
    def test_full_pipeline_dialog_removed(self, bot, fixture_dialog_removed):
        ...
        result = enrich_event(fixture_dialog_removed, bot)

        bot.get_chat_by_id.assert_not_called()
        assert result.chat is None
        assert result.from_user is fixture_dialog_removed.user

    def test_full_pipeline_message_removed_chat_type(
        self, bot, fixture_message_removed
    ):
        fake_chat = _make_chat(ChatType.CHAT)
        fake_member = MagicMock()
    ...
    def test_full_pipeline_message_removed_dialog_type(
        self, bot, fixture_message_removed
    ):
        fake_chat = _make_chat(ChatType.DIALOG)
        bot.get_chat_by_id = MagicMock(return_value=fake_chat)
    ...
    def test_full_pipeline_common_events(self, request, bot, fixture_name):
        ...
        result = enrich_event(event, bot)

        bot.get_chat_by_id.assert_called_once_with(event.chat_id)
        assert result.chat is fake_chat
