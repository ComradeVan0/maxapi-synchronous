...
def test_send_callback_serializes_inline_keyboard_as_attachment(
    mock_bot_token,
):
...
def test_send_callback_omits_attachments_when_not_provided(
    mock_bot_token,
):
    bot = Bot(token=mock_bot_token)
    bot.session = MagicMock()
...
def test_send_callback_serializes_direct_attachment_upload(
    mock_bot_token,
):
    bot = Bot(token=mock_bot_token)
    bot.session = MagicMock()
...
def test_send_callback_puts_disable_link_preview_in_params(
    mock_bot_token,
    value,
    expected,
):
...
def test_send_callback_omits_disable_link_preview_when_none(
    mock_bot_token,
):
    ...
    assert (
        "disable_link_preview" not in mock_request.call_args.kwargs["params"]
    )
...
def test_bot_send_callback_resolves_disable_link_preview(
    mock_bot_token,
    bot_default,
    explicit,
    expected,
