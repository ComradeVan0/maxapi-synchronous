...
def test_get_comments_builds_pagination_params(bot):
    method = GetComments(
        bot=bot,
...
def test_get_comments_by_ids_omits_pagination(bot):
    method = GetComments(
        bot=bot,
        message_id=MESSAGE_ID,
...
def test_get_comments_validates_empty_comment_ids(bot):
    with pytest.raises(ValueError, match="comment_ids"):
        GetComments(bot=bot, message_id=MESSAGE_ID, comment_ids=[])


def test_get_comments_validates_blank_comment_id_element(bot):
...
def test_comment_methods_reject_forward_link(bot, factory):
...
def test_get_comments_converts_datetime_to_milliseconds(bot):
...
def test_get_comments_validates_count(bot, count):
    with pytest.raises(ValueError, match="count"):
        GetComments(bot=bot, message_id=MESSAGE_ID, count=count)


def test_get_comments_validates_message_id(bot):
    with pytest.raises(ValueError, match="message_id"):
        GetComments(bot=bot, message_id="")


def test_get_comment_builds_path(bot):
...
def test_send_comment_builds_json(bot):
    method = SendComment(
        bot=bot,
        message_id=MESSAGE_ID,
        text="привет",
...
def test_send_comment_validates_text_length(bot):
...
def test_send_comment_requires_text_or_link(bot):
    with pytest.raises(ValueError, match="text или link"):
        SendComment(bot=bot, message_id=MESSAGE_ID)


def test_edit_comment_builds_params_and_json(bot):
    method = EditComment(
        bot=bot,
        message_id=MESSAGE_ID,
        comment_id=COMMENT_ID,
...
def test_delete_comment_builds_params(bot):
...
def test_delete_comment_validates_comment_id(bot):
...
def test_methods_validate_empty_message_id(bot, factory):
    with pytest.raises(ValueError, match="message_id"):
        factory(bot)
...
def test_methods_validate_empty_comment_id(bot, factory):
    with pytest.raises(ValueError, match="comment_id"):
        factory(bot)


def test_edit_comment_validates_text_length(bot):
    with pytest.raises(ValueError, match="text"):
        EditComment(
            bot=bot,
            message_id=MESSAGE_ID,
...
def test_edit_comment_with_link_and_string_format(bot):
    method = EditComment(
        bot=bot,
        message_id=MESSAGE_ID,
        comment_id=COMMENT_ID,
...
def test_bot_get_comments_wrapper(bot):
    with _patched_request(Comments(messages=[])) as mocked_request:
        bot.get_comments(message_id=MESSAGE_ID, after=1000)
...
def test_bot_get_comment_wrapper(bot):
    with _patched_request(CommentMessage(**COMMENT_PAYLOAD)) as mocked:
        comment = bot.get_comment(message_id=MESSAGE_ID, comment_id=COMMENT_ID)
...
def test_bot_send_comment_uses_default_format(bot):
    bot.parse_mode = TextFormat.HTML

    with _patched_request() as mocked_request:
        bot.send_comment(message_id=MESSAGE_ID, text="привет")
...
def test_comment_reply_shortcut(bot):
...
def test_comment_edit_shortcut(bot):
    comment = _enriched_comment(bot)

    with _patched_request(EditedComment(success=True)) as mocked_request:
        comment.edit("новый текст")
...
def test_comment_delete_shortcut(bot):
    comment = _enriched_comment(bot)
...
def test_post_message_id_filled_from_recipient(bot):
    payload = {
        **COMMENT_PAYLOAD,
        "recipient": {
            "chat_id": 10,
...
def test_comment_shortcut_without_post_message_id(bot):
    comment = CommentMessage(**COMMENT_PAYLOAD)
    comment.bot = bot
...
def test_comment_shortcut_without_bot():
    comment = CommentMessage(**COMMENT_PAYLOAD)
    comment.post_message_id = MESSAGE_ID
