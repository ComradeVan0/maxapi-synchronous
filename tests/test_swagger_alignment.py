...
def test_get_messages_sends_message_ids_as_comma_list(bot):
    method = GetMessages(
        bot=bot,
...
def test_get_messages_sends_datetime_bounds_as_unix_seconds(bot):
...
def test_get_messages_omits_none_count(bot):
...
def test_add_admin_chat_sends_only_admins_payload(bot):
    admin = ChatAdmin(
        user_id=1,
...
def test_add_admin_chat_ignores_deprecated_marker(bot):
    admin = ChatAdmin(
        user_id=1,
        permissions=[ChatPermission.READ_ALL_MESSAGES],
    )
...
def test_bot_add_list_admin_chat_ignores_deprecated_marker(bot):
    admin = ChatAdmin(
        user_id=1,
        permissions=[ChatPermission.READ_ALL_MESSAGES],
    )
...
def test_get_chat_by_link_normalizes_public_link(
    bot,
    link,
    expected_path,
):
...
def test_get_chat_by_link_keeps_valid_link_characters(bot):
    with pytest.warns(DeprecationWarning, match="GetChatByLink"):
